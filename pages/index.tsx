...
export default function Home() {
  ...
  const toggleToilet = async (device: string) => {
    const serverUrl = process.env.NEXT_PUBLIC_SERVER_URL;
    if (!serverUrl) return;
    const data: { lamp?: number; fan?: number } = {};
    let lamp = toiLamp;
  ...
  useEffect(() => {
    ...
    axios.get(serverUrl).then((res) => {
      ...
      const toiLamp = res.data.kitchen.lamp;
      const toiFan = res.data.kitchen.fan;
      setLivLamp(livLamp);
      setLivFan(livFan);
      setBedLamp(bedLamp);
      setBedFan(bedFan);
        ...
        <div className="container">
          ...
          <div className="grid grid-cols-4 gap-8 mt-6">
            <div className="col-span-3 grid grid-cols-2 gap-8">
              <div className="flex items-center justify-between col-span-2 text-slate-200">
                <div>
                  ...
                  <p className="opacity-70">
                    Welcome home! The air quality is good & Fresh, you can go
                    out today
                  </p>
              ...
              <div className="toilet border border-gray-700/20 rounded-xl p-5 shadow-lg shadow-gray-700/50">
                <div className="flex text-slate-200 items-center mb-4">
                  <FaToilet className="mr-3 text-2xl" />
                  <p className="text-xl text-slate-200">Toilet </p>
                </div>
                ...
                <div className="flex">
                  <div className="w-48 h-32 rounded-2xl bg-gradient-to-t from-green-600 to-emerald-400 mr-5">
                    <div className="p-6 text-slate-200 m-auto flex justify-between h-full">
                      <div className="flex flex-col justify-between">
                        <LightBulbIcon
                          className={`h-8 w-8 ${
                            toiLamp ? "text-yellow-300" : "text-slate-200"
                          }`}
                        />
                        <p className="font-bold">Lamp</p>
                      </div>
                      <div className="relative">
                        <div
                          onClick={() => toggleToilet("lamp")}
                          className="hover:cursor-pointer"
                        >
                          <div className="block bg-gray-600 w-12 h-6 rounded-full"></div>
                          <div
                            className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-all ${
                              toiLamp ? "bg-green-300 translate-x-6" : ""
                            }`}
                          ></div>
                        </div>
                      </div>
                  ...
                  <div className="w-48 h-32 rounded-2xl bg-gradient-to-t from-cyan-500 to-blue-400">
                    <div className="p-6 text-slate-200 m-auto flex justify-between h-full">
                      ...
                      <div className="relative">
                        <div
                          onClick={() => toggleToilet("fan")}
                          className="hover:cursor-pointer"
                        >
                          <div className="block bg-gray-600 w-12 h-6 rounded-full"></div>
                          <div
                            className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-all ${
                              toiFan ? "bg-green-300 translate-x-6" : ""
                            }`}
                          ></div>
                        </div>
                      </div>
